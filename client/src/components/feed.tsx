...
export default function Feed() {
  ...
  return <main className="feed-page"><aside className="feed-sidebar"><div className="feed-brand"><Link to="/" aria-label="RSSFeed home"><span className="feed-rss">◔</span> rssfeed</Link></div><button className="new-feed" type="button" onClick={() => setIsAdding(true)}><span>+</span> Add a feed</button><p className="sidebar-label">YOUR FEEDS</p><div className="feed-list"><button className="selected-feed" type="button"><span className="feed-dot coral" /><span>The Daily Brief</span><small>0</small></button></div><div className="sidebar-bottom"><p><b>One feed is free.</b><br />Sign up to save feeds across devices.</p><Link to="/signup" className="outline-signup">Create an account</Link></div></aside><section className="feed-main"><header className="feed-header"><div><p className="feed-kicker">YOUR FEED</p><h1>The Daily Brief</h1></div><div className="account-actions">{name ? <span className="signed-in-name">Hi, {name}</span> : <><Link to="/login" className="login-link">Log in</Link><Link to="/signup" className="signup-link">Sign up</Link></>}</div></header><div className="empty-feed"><div className="empty-icon"><span>+</span></div><p className="feed-kicker">YOUR READING SPACE</p><h2>Your feed is ready.</h2><p>Add an RSS source to start seeing stories here. You can try one feed without creating an account.</p><button type="button" className="add-source-button" onClick={() => setIsAdding(true)}>Add your first source <span>→</span></button></div></section>{isAdding && <div className="modal-backdrop"><form className="feed-modal" onSubmit={submitFeed}><button className="close-modal" type="button" onClick={() => setIsAdding(false)}>×</button><p className="feed-kicker">ADD A SOURCE</p><h2>Make it part of your feed.</h2><label htmlFor="feed-name">Feed name</label><input id="feed-name" value={feedName} onChange={(event) => setFeedName(event.target.value)} placeholder="e.g. The Verge" /><label htmlFor="feed-url">RSS URL</label><input id="feed-url" value={feedUrl} onChange={(event) => setFeedUrl(event.target.value)} placeholder="https://example.com/feed.xml" />{error && <p className="form-error">{error}</p>}<button className="modal-submit" disabled={isSubmitting} type="submit">{isSubmitting ? "Adding..." : "Add feed"} →</button></form></div>}</main>
}
